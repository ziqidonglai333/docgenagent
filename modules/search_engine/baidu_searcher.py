...
class BaiduSearcher:
    ...
    def __init__(self):
        self.config = SEARCH_ENGINE_CONFIG["baidu"]
    ...
    def _parse_search_results(self, html: str, search_term: str) -> List[Dict[str, Any]]:
        ...
        # 百度搜索结果通常包含在class为"result"或"c-container"的div中
    ...
    def _extract_result_info(self, result_div, search_term: str) -> Dict[str, Any]:
        ...
        # 解析真实URL（百度使用重定向）
        real_url = self._resolve_baidu_url(url)
        if not real_url:
            return None
        
        # 提取摘要
        abstract_elem = result_div.find('div', class_='c-abstract') or result_div
